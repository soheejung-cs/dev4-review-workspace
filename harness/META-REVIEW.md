...
## 부정 결과 — PR 난이도 기반 모델 자동 지정은 지금 근거가 없다 (2026-09-21)

사용자 요청("모델 자동 지정 기능도 넣을 수 있나")을 조사했고, **만들지 않기로 했다.** 근거는 실측이다.

### 무엇을 재 봤나

`tools/review_recommend.py` 의 `difficulty()` 를 **머지 PR 100건**(`merged:>=2026-04-01`)에 돌려
리뷰 노력 대리지표(봇 제외 리뷰 스레드 수·사람 리뷰어 수·커밋 수)와 대조했다.

| 비교 | ρ(스피어만) |
|---|---|
| difficulty 점수 vs **파일 수** | **0.865** |
| difficulty 점수 vs 사람 리뷰 스레드 | 0.589 |
| **줄 수만** vs 사람 리뷰 스레드 | 0.501 |
| difficulty 점수 vs 커밋 수 | 0.634 (**줄 수만 쓰면 0.749 — 점수가 더 나쁘다**) |

**이 점수기는 규모 센서다.** 7개 규칙 전체가 "줄 수 세기" 대비 스레드 예측을 0.501→0.589 만큼만 개선하고,
리비전 수는 오히려 줄 수만 쓰는 쪽이 낫다. 점수 분해: 규모축 65% / 판단축 35%.

### 오분류가 양방향으로 실재한다

- **작은데 어려운 것이 '간단' 으로**: #7243 race condition(12줄, **0점**, 스레드 5) · #7817 parallel scan 임계(**4줄**, 1점, 리뷰어 3) ·
  #7848/7849/7850 메모리 오버리드 수정(**각 1줄**, 2점).
- **큰데 기계적인 것이 '어려움' 으로**: #7803 머지 준비(+28929, 9점, **스레드 0**) · #7671 라이브러리 교체(9점, 스레드 0).
  tier '어려움' 33건 중 **스레드 0 이 10건(30%)**.
- **같은 변경의 백포트가 tier 가 갈린다** — #7863(4점 보통) vs #7933 백포트(2점 간단).

### 그래서 한 일 / 하지 않은 일

- **하지 않음**: 난이도 → 모델 매핑. 입력이 규모를 재고 있어 "어려운 PR 에 좋은 모델" 이 되지 않는다.
- **함**: 단계 성격 기반 선택([모델-선택.md](모델-선택.md)) — 수집은 낮추고 판정·종합·반증은 유지.
  이 축은 PR 난이도와 무관하므로 위 반증의 영향을 받지 않는다.
- **함**: 반증이 드러낸 `difficulty()` 의 매칭 결함을 고쳤다(아래).

### 부수 수확 — 리뷰어 추천에 실제로 영향을 주던 결함

부분문자열 매칭이 오늘 실제 추천을 틀리게 하고 있었다.

| 오검출 | 결과 |
|---|---|
| `hard_files` 의 `log_` 가 `cata`**`log_`**`install.cpp` 에 매칭 | **PR#7976 이 '보통 2명' 으로 추천됐다. 고친 뒤 '간단 1명'** |
| `hard_title` 의 `lock` 이 `CLOCK_MONOTONIC`·`string block search` 에 | 무관한 PR 이 +1 |
| `easy_title` 의 `comment` 가 "post a PR comment" 에 | 무관한 PR 이 −1 |
| `hard_title` 에 `race` 가 **없었다** | race condition PR 이 0점 |

수정: ASCII 키워드는 **단어 경계**로, `log_`·`lock_` 처럼 밑줄로 끝나는 파일 키는 **basename 접두**로,
나머지 파일 키는 토큰 경계로. `race`·`barrier`·`atomic`·`volatile`·`경쟁` 추가. (한글 키워드는 단어 경계 개념이 없어 부분 일치 유지.)

### 이 반증의 한계 (정직하게)

리뷰 스레드·커밋·리뷰어 수는 **투입된 관심**이지 **내재 난이도**가 아니다. 하네스가 실제로 결함을 놓친 #7899 는
스레드 18 로 높은 쪽이고, #7977 은 스레드 0 인데 그건 "쉬웠다" 가 아니라 "아직 사람이 안 봤다" 다.
따라서 위 수치는 **"점수기가 규모를 재고 있다"** 를 강하게 보이지만 **"어느 PR 이 정말 어려운가"** 의 정답표는 아니다.
그 정답표는 이 문서의 「놓친 지적 사례」를 쌓아 사후 대조로만 만들 수 있다.
